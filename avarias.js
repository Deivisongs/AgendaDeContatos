...
// ALTERADO: Aponta exclusivamente para a store de avarias solicitada
const TARGET_STORE = "avariasStore";
  ...
  if (indexParam !== null) {
    ...
    if (encontrado) {
      ...
      if (contagem.status !== "aberto" && contagem.status !== "open") {
        mostrarModalAviso(
          "Aviso",
          "Este balanço de avarias está ENCERRADO. Para alterar os itens, reabra-o na tela inicial.",
        );
        bloquearAcoesColeta();
      }
    }
  } else {
    const contagemIniciada = await getObjeto("avariaIniciada");
    console.log(contagemIniciada)
    const dataeHora = dataHora();
    lblBalanceDate.textContent = `Data de abertura: ${dataeHora}`;

    if (contagemIniciada && contagemIniciada.loja) {
      lblBalanceName.textContent = contagemIniciada.loja;
      contagem = {
        name: contagemIniciada.loja,
...
async function salvaContagem(dadosContagem) {
  const db = await abrirBanco();
  return new Promise((resolve, reject) => {
    // ALTERADO: Direcionado para avariasStore
...
function renderList() {
  ...
  contagensRealizadas.forEach((item, index) => {
    ...
    // RESOLVIDO E ALTERADO: Busca a descrição correspondente para destacar
    let descExibicao = "Produto Desconhecido";
    if (item.descricaoManual) {
      descExibicao = item.descricaoManual;
    } else if (produtosDatabase[item.codigo]) {
      descExibicao = produtosDatabase[item.codigo].descricao;
    }

    // ALTERADO: Layout focado na Descrição principal com Código menor (coleta-subcode)
    row.innerHTML = `
      <div class="coleta-info">
        <div class="coleta-code">${descExibicao}</div>
        <div class="coleta-subcode">Cód: ${item.codigo}</div>
...
function deleteItem(index) {
  mostrarModalConfirmacao(
    "Remover Item",
    "Tem certeza que deseja remover este item da contagem de avarias?",
    async () => {
...
function handleCodeDetected(code) {
  ...
  if (produtosDatabase[code]) {
    ...
    document.getElementById("titleModalFound").innerText = "Avaria Encontrada";
...
document
  .getElementById("btnSaveNotFound")
...
document.getElementById("btnScan").addEventListener("click", async () => {
  ...
  if (isIphone()) {
    ...
    codeReaderZXing
      .decodeFromConstraints(constraints, "videoPreview", (result, err) => {
